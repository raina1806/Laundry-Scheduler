...
import pytz  # Import pytz module for time zone support
...
user_assignments = {machine_id: {} for machine_id in range(1, num_machines + 1)}  # Start from machine 1
completed_users = []
...
async def assign_machine(request: UserRequest):
    ...
    current_time = datetime.now(pytz.timezone('Asia/Kolkata'))  # Set time zone to Asia/Kolkata
    new_user = {
        "user_name": user_name,
        "wash_time": wash_time,
    ...
    for machine_id in range(1, num_machines + 1):  # Start from machine 1
    ...
    if user_queues.qsize() == 1:
        shortest_wash_time = min(user_assignments.values(), key=lambda x: x.get("wash_time", 0)).get("wash_time", 0)
...
async def check_status():
    ...
    # Update all statuses
    for i, machine_user in user_assignments.items():  # Iterate over machine IDs
        if machine_user:
    ...
    for user in user_queues.queue:
        wash_time = user.get("wash_time", 0)
    ...
    if user_queues.qsize() > 0:
        ...
        ).strftime("%I:%M %p")  # Calculate machine availability time
        user_queues.queue[0]["machine_available_at"] = machine_available_at
